...
/* Phase timings for burst mode, interstim, p1, interphase, p2, interburst */
...
int command_burst_phase_one(uint32_t param);
int command_burst_phase_two(uint32_t param);
int command_burst_phase_gap(uint32_t param);
int command_burst_stim_gap(uint32_t param);
int command_burst_burst_gap(uint32_t param);
int command_burst_pulse_num(uint32_t param);
int command_burst_burst_num(uint32_t param);
...
static int (*command_handlers[])(uint32_t param) = {
    ...
    command_burst_phase_one,
    command_burst_phase_two,
    command_burst_phase_gap,
    command_burst_stim_gap,
    command_burst_burst_gap,
    command_burst_pulse_num,
    command_burst_burst_num,
    command_burst_phase_one_curr,
    command_burst_phase_two_curr,
    command_dc_ramp_time,
    command_dc_hold_time,
    command_dc_curr_target,
};
...
int command_anodic_cathodic(uint32_t param) {
  ...
  if (param == 0 || param == 1) {
    anodic = param;
    burst_vdac_values[1] = INVERT_VDAC(burst_vdac_values[1]);
    burst_vdac_values[3] = INVERT_VDAC(burst_vdac_values[3]);
    return 0;
  } else {
    return 1;
  }
...
int command_burst_phase_one(uint32_t param) {
  burst_phase_timings[1] = US_TO_PT(param);
  return 0;
}

int command_burst_phase_two(uint32_t param) {
  burst_phase_timings[3] = US_TO_PT(param);
  return 0;
}

int command_burst_phase_gap(uint32_t param) {
  burst_phase_timings[2] = US_TO_PT(param);
  return 0;
}

int command_burst_stim_gap(uint32_t param) {
  burst_phase_timings[0] = US_TO_PT(param);
  return 0;
}

int command_burst_burst_gap(uint32_t param) {
  burst_phase_timings[4] = US_TO_PT(param);
  return 0;
}
